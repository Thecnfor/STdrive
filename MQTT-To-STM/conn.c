...
static void MQTT_Log(const char *fmt, ...) {
  ...
  HAL_UART_Transmit(MQTT_LOG_UART_HANDLE, (uint8_t *)log_buf, strlen(log_buf),
                    100);
}
#else
 ...
 * @return false 超时或未找到
 */
static bool ESP_Execute(const char *cmd, const char *expected, char *out_buf,
                        uint16_t buf_len, uint32_t timeout_ms) {
...
static bool ESP_SendAT(const char *cmd, const char *expected,
                       uint32_t timeout_ms) {
    ...
    if (MQTT_Process(topic, sizeof(topic), payload, sizeof(payload))) {
      ...
      for (int i = 0; i < subscription_count; i++) {
        if (subscriptions[i].callback &&
            MQTT_TopicMatched(subscriptions[i].topic, topic)) {
...
bool MQTT_IsConnected(void) { return is_connected; }
...
bool MQTT_Start(void) {
  ...
  if (ESP_Execute("AT+CMD?\r\n", "OK", buf, RX_BUFFER_SIZE,
                  AT_CMD_TIMEOUT_SHORT)) {
    MQTT_Log("CMD Response: %s\r\n", buf);
  } else {
    MQTT_Log("CMD Timeout/Error: %s\r\n", buf);
  }
  /* 检查是否已连接目标 WiFi */
  /* 发送 AT+CWJAP? 并检查响应中是否包含 SSID */
  bool wifi_connected = false;
  if (ESP_Execute("AT+CWJAP?\r\n", "OK", buf, RX_BUFFER_SIZE,
                  AT_CMD_TIMEOUT_NORMAL)) {
  ...
  if (!wifi_connected) {
    ...
    if (!ESP_SendAT(cmd_buf, "OK", AT_CMD_TIMEOUT_WIFI)) {
      ...
      // 简单起见，如果超时或无 OK 视为失败
      // 注意：部分固件返回 WIFI CONNECTED 后才返回 OK，或者只返回 WIFI
      // CONNECTED 严谨做法应检查 buf
      MQTT_Log("WiFi 连接失败\r\n");
    } else {
      MQTT_Log("WiFi 连接成功\r\n");
    }
...
bool MQTT_SubscribeCallback(const char *topic, MQTT_MessageHandler handler) {
  /* 检查是否已存在 */
  for (int i = 0; i < subscription_count; i++) {
    if (strncmp(subscriptions[i].topic, topic,
                sizeof(subscriptions[i].topic)) == 0) {
      /* 更新回调函数 */
  ...
  if (subscription_count < MAX_SUBSCRIPTIONS) {
    strncpy(subscriptions[subscription_count].topic, topic,
            sizeof(subscriptions[subscription_count].topic) - 1);
    subscriptions[subscription_count]
        .topic[sizeof(subscriptions[subscription_count].topic) - 1] = '\0';
...
void MQTT_SetSubscriptions(const MQTT_SubscribeInfo *list) {
  ...
  for (int i = 0; i < subscription_count;) {
    bool found = false;
    /* 遍历新列表查找是否存在 */
    for (int j = 0; list[j].topic != NULL; j++) {
      if (strncmp(subscriptions[i].topic, list[j].topic,
                  sizeof(subscriptions[i].topic)) == 0) {
        found = true;
        break;
      }
    ...
    if (!found) {
      ...
      strncpy(topic_to_remove, subscriptions[i].topic,
              sizeof(topic_to_remove) - 1);
      topic_to_remove[sizeof(topic_to_remove) - 1] = '\0';

      MQTT_Unsubscribe(topic_to_remove);
      /* MQTT_Unsubscribe 成功移除后，后续元素前移，当前索引 i
       * 指向新元素，故无需 i++ */
    } else {
      /* 在新列表中，保留 */
      i++;
...
static void OnLedControl(const char *topic, const char *payload) {
  MQTT_Log("[Callback] 收到 LED 控制: %s\r\n", payload);
  /* 用户代码：转发到串口 */
  /* 注意：payload 是 const char*，需要转换为 uint8_t*，且去掉了 const */
...
bool MQTT_Process(char *topic, uint16_t topic_size, char *payload,
      ...
      if (sscanf(ipd_ptr, "+IPD,%d:", &len) == 1) {
        ...
        if (rx_idx >= ipd_offset + total_len) {
          ...
          if ((mqtt_data[0] & 0xF0) == 0x30) {
            ...
            /* Topic Length */
            uint16_t t_len = (mqtt_data[var_header_start] << 8) |
                             mqtt_data[var_header_start + 1];
            ...
            if (topic != NULL && topic_size > 0) {
              uint16_t copy_len =
                  (t_len < topic_size) ? t_len : (topic_size - 1);
            ...
            if (payload != NULL && payload_size > 0) {
              uint16_t copy_len =
                  (p_len < payload_size) ? p_len : (payload_size - 1);
